package net.joeruPOGsan.translationmod.TranslationPipeLine.api;

import com.google.gson.JsonArray;
import com.google.gson.JsonElement;
import com.google.gson.JsonParser;
import net.joeruPOGsan.translationmod.TranslationPipeLine.http.HttpClient4;
import net.joeruPOGsan.translationmod.TranslationPipeLine.http.ParamPairList;
...
public class GoogleTransAPI {
     ...
     */
    public GoogleTransAPI(HttpClient4 httpClient4){
    ...
    private List<String> transJsonSerialization(String jsonResult, boolean isSingle) {
        ...
        if (isSingle) {
            resultList.add(resultArray.get(0).getAsString());
        } else {
            for (JsonElement item : resultArray) {
                JsonArray innerArray = item.getAsJsonArray().get(0).getAsJsonArray();
                if (!innerArray.isEmpty()) {
                    resultList.add(innerArray.get(0).getAsString());
                }
            }
        }
